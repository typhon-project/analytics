...
public class TopMerchantsSum extends StreamAnalyzer {
	...
	public DataStream<Event> analyse(DataStream<Event> postEvents)
		...
		DataStreamSink<Tuple3<String, String, Double>> results = postEvents
				...
				.keyBy("MRCH_NAME").timeWindow(Time.days(30)).sum("FNC_EV_AMT")
				.map(new MapFunction<FNC_EV, Tuple3<String, String, Double>>() {

					@Override
					public Tuple3<String, String, Double> map(FNC_EV arg0)
							throws Exception {
						Tuple3<String, String, Double> result = new Tuple3<String, String, Double>();
						String month = arg0.getFNC_EV_DT().toLocalDate()
								.getMonth().toString();
						result.f0 = arg0.getMRCH_NAME();
						result.f1 = month;
						result.f2 = arg0.getFNC_EV_AMT();
						System.out.println(arg0.getMRCH_NAME() + " " + month
								+ " " + arg0.getFNC_EV_AMT());
						return result;
					}
				})
