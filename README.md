...
At this stage, Flink is ready to receive the **non-fat** Typhon Analytics user JAR (i.e. created in the `target` directory of your Typhon Analytics user project and by running `mvn clean package`). An example is listed below:
`docker cp target/ac.york.typhon.analytics.test.scenario1.analytics-0.0.1-SNAPSHOT-jar-with-dependencies.jar $(docker ps --filter name=jobmanager --format={{.ID}}):/opt/flink/usrlib/ac.york.typhon.analytics.test.scenario1.analytics-0.0.1-SNAPSHOT-jar-with-dependencies.jar` 

Next, connect to the Flink Jobmanager Docker container:
`docker exec -it $(docker ps --filter name**=**jobmanager --format**={{**.ID**}}**) /bin/sh`

Finally, run Flink job execution command:
`flink run /opt/flink/usrlib/ac.york.typhon.analytics.test.scenario1.analytics-0.0.1-SNAPSHOT-jar-with-dependencies.jar`  

The Typhon Analytics user JAR has been submitted successfully to Flink if the Flink job execution command (see above) produces an output similar to:
`Job has been submitted with JobID 6c5279ee74b66c469ce8dd4492fbcde1`
